...
이 문서는 프로젝트를 진행하면서 GitHub Issue, Branch, Commit, Pull Request, 배포 흐름을 일관되게 사용하기 위한 협업 규칙입니다.
...
배포 흐름도 브랜치 전략과 동일하게 단순하게 유지합니다.
...
작업 브랜치에서 Pull Request를 생성하면 Vercel Preview Deployment가 자동 생성됩니다. PR 작성자는 Preview URL에서 화면과 기능을 확인한 뒤 리뷰를 요청합니다.

`main`에 병합되면 Vercel Production Deployment가 자동 실행됩니다. 따라서 `main` 병합 전에는 코드 리뷰뿐 아니라 Preview 환경에서의 동작 확인까지 완료되어야 합니다.
...
UI, API 연동, 라우팅, 인증, 환경변수와 관련된 작업은 PR 본문에 아래 항목을 포함합니다.

```md
## 배포 확인

- [ ] Vercel Preview Deployment 성공
- [ ] Preview URL에서 주요 화면 또는 기능 확인
- [ ] 모바일 화면 확인
- [ ] 브라우저 콘솔 에러 없음
- [ ] 환경변수 하드코딩 없음
```

문서 수정처럼 배포 확인이 필요하지 않은 작업은 해당 항목에 `해당 없음`이라고 작성합니다.

## 9. 코드 리뷰 규칙

* PR 작성자는 리뷰어가 이해할 수 있도록 변경 내용을 간단히 설명합니다.
* PR 작성자는 Vercel Preview URL에서 실제 동작을 확인한 뒤 리뷰를 요청합니다.
* 리뷰어는 동작 여부, 코드 구조, 불필요한 코드 여부를 확인합니다.
* 리뷰어는 필요한 경우 Vercel Preview URL에서 화면과 기능을 직접 확인합니다.
...
* Vercel Preview Deployment가 실패하지 않았는가
...
`main`에 병합되면 Vercel Production Deployment가 자동 실행됩니다. Production 배포가 실패하면 해당 PR 작성자가 우선 원인을 확인하고, 필요한 경우 팀장과 함께 수정합니다.
...
6. 원격 Branch에 Push
7. Pull Request 생성
8. Vercel Preview Deployment 확인
9. 코드 리뷰 및 수정
10. main 브랜치에 Merge
11. Vercel Production Deployment 확인
12. 작업 Branch 삭제
```
...
7. Vercel Preview URL에서 로그인 화면과 로그인 실패 케이스 확인
...
* Production과 Preview에서 사용하는 환경변수가 다를 수 있으므로, 배포 오류가 발생하면 환경변수 누락 여부를 먼저 확인합니다.
* 큰 작업은 작은 Issue로 나눕니다.
* 단순 수정이라도 가능하면 Issue와 PR 기록을 남깁니다.
* 이해하지 못한 코드는 그대로 병합하지 않습니다.
* GitHub와 Vercel에 남는 기록은 포트폴리오와 협업 과정의 근거가 되므로, 제목과 설명을 최소한의 수준으로라도 명확하게 작성합니다.
