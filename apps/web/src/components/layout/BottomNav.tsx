...
function getActiveTab(pathname: string): TabId | null {
  ...
  if (matchesRoute(pathname, ROUTES.locationPermission)) return "questions";
...
export function BottomNav() {
  ...
  const handleTabClick = (tab: { id: TabId; route: string }) => {
    if (tab.id === activeTab || isChecking) return;
    if (tab.id === "questions") {
      startRecommendationFlow();
      return;
    }
